"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CliniquesService = void 0;
const common_1 = require("@nestjs/common");
const client_1 = require("@prisma/client");
const prisma_service_1 = require("../prisma/prisma.service");
let CliniquesService = class CliniquesService {
    constructor(prisma) {
        this.prisma = prisma;
    }
    findAll() {
        return this.prisma.clinique.findMany({ orderBy: { nom: 'asc' } });
    }
    async findOne(id) {
        const clinique = await this.prisma.clinique.findUnique({
            where: { id },
            include: { _count: { select: { personnel: true, services: true } } },
        });
        if (!clinique)
            throw new common_1.NotFoundException('Clinique introuvable.');
        return clinique;
    }
    async create(dto) {
        try {
            return await this.prisma.clinique.create({ data: dto });
        }
        catch (e) {
            if (e instanceof client_1.Prisma.PrismaClientKnownRequestError && e.code === 'P2002') {
                throw new common_1.ConflictException('Ce code de clinique existe déjà.');
            }
            throw e;
        }
    }
    async update(id, dto) {
        await this.findOne(id);
        try {
            return await this.prisma.clinique.update({ where: { id }, data: dto });
        }
        catch (e) {
            if (e instanceof client_1.Prisma.PrismaClientKnownRequestError && e.code === 'P2002') {
                throw new common_1.ConflictException('Ce code de clinique existe déjà.');
            }
            throw e;
        }
    }
    async remove(id) {
        await this.findOne(id);
        return this.prisma.clinique.update({
            where: { id },
            data: { statut: 'INACTIF' },
        });
    }
};
exports.CliniquesService = CliniquesService;
exports.CliniquesService = CliniquesService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService])
], CliniquesService);
//# sourceMappingURL=cliniques.service.js.map